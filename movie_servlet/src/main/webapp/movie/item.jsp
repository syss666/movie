...
<script>
		...
		$.ajax({
			...
			{	
				if(s_data=='find'){
					...
					$('#fav_a').attr("href", "javascript:fav_delete()");
				}	
			}
		});	
	}
	...
	function fav_delete()//관심영화 저장 중복 확인
	{			
		$.ajax({
		    url: "/moviefav_delete.do",
		    type: "GET",     
		    data: { "movieid":m_id},
			success:function(s_data) 
			{
				$('#fav_img').attr("src", "/movie/img/checkmark-outline.svg");
				$('#fav_a').attr("href", "javascript:fav_insert()");
			}
		});	
	}
